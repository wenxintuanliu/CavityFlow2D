...
# ==============================================================================
    ...
    if st.session_state.cfd_result:
        ...
        with plot_cols[0]:
            ...
            layout.render_plot_with_caption(fig1, "速度幅值 (Velocity Magnitude)", "#e7f5ff")
        with plot_cols[1]:
            fig2 = plots.plot_streamlines(res['u'], res['v'], res['grid'], res['re'])
            layout.render_plot_with_caption(fig2, "流线图 (Streamlines)", "#fff3bf")
        with plot_cols[2]:
            fig3 = plots.plot_pressure(res['p'], res['grid'], res['re'])
            layout.render_plot_with_caption(fig3, "压力场 (Pressure Field)", "#ffe3e3")
    else:
